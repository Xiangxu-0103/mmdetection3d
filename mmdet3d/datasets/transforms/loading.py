...
from mmdet.datasets.transforms import LoadAnnotations, LoadEmptyAnnotations
...
@TRANSFORMS.register_module()
class LoadEmptyAnnotations3D(LoadEmptyAnnotations):
    """Load Empty Annotations3D for unlabeled images.
    ...
    def __init__(self,
                 ...
                 with_seg: bool = False,
                 with_bbox_depth: bool = False,
                 with_panoptic_3d: bool = False) -> None:
        super().__init__(
            with_bbox=with_bbox,
            with_label=with_label,
            with_mask=with_mask,
    ...
    def transform(self, results: dict) -> dict:
        ...
        results = super().transform(results)
        if self.with_bbox_3d:
            results['gt_bboxes_3d'] = np.zeros((0, 7), dtype=np.float32)
        if self.with_bbox_depth:
            results['depths'] = np.zeros((0, ), dtype=np.float32)
            results['centers_2d'] = np.zeros((0, 2), dtype=np.float32)
        if self.with_label_3d:
            results['gt_labels_3d'] = np.zeros((0, ), dtype=np.int64)
        if self.with_attr_label:
            results['attr_labels'] = np.zeros((0, ), dtype=np.int64)
        if self.with_panoptic_3d:
            num_points = results['points'].shape[0]
            results['pts_semantic_mask'] = np.zeros((num_points, ),
                                                    dtype=np.int64)
            results['pts_instance_mask'] = np.zeros((num_points, ),
                                                    dtype=np.int64)
        if self.with_mask_3d:
            num_points = results['points'].shape[0]
            results['pts_instance_mask'] = np.zeros((num_points, ),
